function [A Ag] = get_tm_smooth(mus,xPath)
...
for n=1:N;
    ...
    [ig xmap] = min(abs(xPath{n}*ones(1,K)-mus(ones(length(xPath{n}),1),:)),[],2);
    xdiff = find(diff(xmap)~=0) + 1;
    for i=1:(length(xdiff)-1)
        k=xmap(xdiff(i));
        count(k) = count(k)+1;
        dwells{k}(count(k)) = xdiff(i+1)-xdiff(i);
        dest{k}(count(k)) = xmap(xdiff(i+1));
        if xmap(xdiff(i))==xmap(xdiff(i+1))
            warning('wtf')
            keyboard
        end
    end
...
for k=1:K
   ...
   % the normalized cumulative histog   ram can is described by exp(-k*t), but it
