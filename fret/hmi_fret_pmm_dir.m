function runs = hmi_fret_pmm_dir(x, hf_runs, M_values, K, varargin)
    ...
    try
        ...
        if (opts.K < K_max) & opts.use_majority_states
            ...
            [vb u idxs] = majority_states([hf_runs(i_max).vb.w], ...
                                          hf_runs(i_max).u, ...
        ...
        for m = 1:length(M_values)
            ...
            runs(m).u0 = init_u_pmm_dir(M_values(m), vb, u);
        end

        % set initial state priors to uninformative 
        % (we don't want to bias subpopulation size results)
        for m = 1:length(runs)
            runs(m).u0.pi = ones(size(runs(m).u0.pi));
        end
