...
root_fun = @(nu) E_log_g ...
                 + 0.5 * ((1 ./ (nu - 1)) ...
                          ...
                          + log(pi .* nu ./ E_l) ...
                          - psi(nu / 2));
opts = optimset('display', 'off', 'tolX', 1e-6, 'tolFun', 1e-16);
...
E_log_pi = arrayfun(@(w) psi(w.pi) - psi(sum(w.pi)), ...
                    w, 'UniformOutput', false);
E_log_pi = mean([E_log_pi{:}], 2);
wpi = exp(E_log_pi);

%(pi): solve system of equations
root_fun = @(p) wpi - exp(psi(p) - psi(sum(p)));
...
E_log_wA = arrayfun(@(w) w.wt * bsxfun(@plus, psi(w.A), -psi(sum(w.A,2))), ...
                    w, 'UniformOutput', false);
...
for k = 1:K
    ...
    %               opts);
    % u.A(k,:) = a0 * wA(k,:);
    root_fun = @(Ak) wA(k,:) - exp(psi(Ak) - psi(sum(Ak)));
    u.A(k,:) = lsqnonlin(root_fun, ...
                         u_old.A(k,:), ...
                         eps * ones(size(u_old.A(k,:))), ...
