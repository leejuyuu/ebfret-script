...
E_log_pi = arrayfun(@(w) psi(w.pi + eps) - psi(sum(w.pi + eps)), ...
                    w, 'UniformOutput', false);
E_log_pi = mean([E_log_pi{:}], 2);
w_pi = exp(E_log_pi);

%(pi): solve system of equations
%root_fun = @(u_pi) E_log_pi - (psi(u_pi) - psi(sum(u_pi)));
root_fun = @(u_pi) w_pi - (psi(u_pi) - psi(sum(u_pi)));
...
for k = 1:K
    ...
    root_fun = @(u_Ak) w_A(k,:) - exp(psi(u_Ak) - psi(sum(u_Ak)));
    u.A(k,:) = lsqnonlin(root_fun, ...
                         u_old.A(k,:), ...
                         zeros(size(u_old.A(k,:))) + 1e-6, ...
